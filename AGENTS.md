...
| `/release` | 發布正式版（確認授權、公告版號四處同步、**上版前資安檢測**、開 dev→main PR、CI 綠燈後合併、確認 Actions 部署） |
| `/verify-app` | 驗證改動（vitest、dev server 實測、排版問題先定層再動手） |

Hooks（共用腳本在 `scripts/hooks/`，由 `.codex/hooks.json` 掛載；路徑由 Git 專案根目錄解析，不得寫死本機絕對路徑）：
- **guard-main-push**：任何推送 main 或合併 dev→main PR 的指令（git push／merge API／`gh pr merge`）一律先跳使用者確認
...
資安防線（2026-07-11 起）：CI（ci.yml）含 `npm audit --audit-level=high` 依賴弱掃；發正式版前 `/release` 步驟 3 必須依明確 checklist 審查整包待發 diff；並非所有工具都提供同名 `/security-review` 指令，發版驗收必須以明確 checklist 與實際證據為準（Claude Code 有內建 `/security-review`，可作為 checklist 之外的第二道檢查，但不可互相取代）。
...
- **`dev` 分支**：所有開發與修復一律先推到這裡，Vercel 自動部署測試版（分支保護：禁止 force push／刪除）
- **`main` 分支**：只能經 **dev → main 的 PR** 更新（required status check `test-build` 過了才能合併；`enforce_admins` 開啟，本機直推對管理者也會被拒），且合併 PR 前必須取得使用者**明確確認**
- **絕對不可在未經確認的情況下更新 `main`**（合併 PR＝發布正式版）
...
### 發布正式版（使用者確認後才執行）
正式版由 **GitHub Actions**（`.github/workflows/deploy.yml`）自動 build＋部署——來源 `auris-vue/dist`，**不再手動 build／copy 到根目錄、也不需清舊 hash**（自 2026-05-20 起如此，P97-P100 整理批次移除了根目錄冗餘成品）。

發布走 **dev → main PR**（2026-07-18 起，詳細指令見 `/release` skill）：
1. 確認 dev 最新 commit 的 CI（`test-build`）綠燈
2. 開（或沿用）dev → main 的 PR
3. required check 通過、`mergeable_state` 為 `clean` 後，**取得使用者明確同意**再合併 PR
4. 合併觸發 deploy.yml 自動部署 GitHub Pages 正式版

**推 `dev` 屬日常流程，不需逐次確認；合併 dev→main PR＝發布正式版，必須先取得使用者明確同意（guard-main-push hook 對 merge API／`gh pr merge`／直推 main 都會再強制確認一次）。本機 `git merge dev && git push origin main` 已被遠端 required checks 封鎖，不要再用。**
