...
- **「清空」改為自訂確認 modal（含勾選框）**：取代原本資訊不足的 native `confirm()`。modal 明確標示「角色不會被刪除」，並提供勾選框「同時清除日記、夢境、貼文」，**預設不勾**——只清聊天訊息與對話記憶，避免「我只想清訊息結果日記也沒了」的二次誤刪；想徹底重新開始的使用者可主動勾選連帶清除。勾選時會一併清掉這些內容對應的通知（type `post/diary/dream`），避免通知點進去連到已刪內容。
