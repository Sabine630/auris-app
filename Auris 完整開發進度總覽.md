...
- 🟢 **世界觀設定書 World Book 📖** — 與角色脫鉤的客觀詞條庫，供 AI 參考
- 🟢 **定位系統 📍 ＋ 任務系統 ✓** — 每日登入與互動的遊戲化機制
- 🟢 **劇本／小說體驗 🎭** — AVG 選項分支與長篇協作創作模組

#### 使用者反饋待辦（上線後收集，排入階段 B）

- 🟡 **角色與玩家作息設定** — 上班時間、地點欄位，讓主動訊息有情境感（`CharEditView` / `MeView` / `chatEngine` system prompt）
- 🟡 **自動總結記憶** — 在現有手動總結外，加「每 X 則訊息自動觸發」（`ChatRoomView` / `chatEngine`）
- 🟡 **玩家自訂大頭貼** — 玩家可上傳／更換自己的頭像（`MeView`）
- 🟡 **圖片傳送與 AI 識別** — 對話支援傳圖、多模態識別（需確認模型支援 vision）

### 🟣 階段 C：系統底層重構（長期）

- 🔵 **多世界系統 🌍** — IndexedDB 全域加入 `worldId` 隔離資料（`worlds` 表與 `characters.worldId` 索引已預留）
- 🔵 **完整 PWA 離線體驗與推播** — Service Worker 快取、系統級推播，評估上架 App Store / Google Play
...
路由    Vue Router 4（createWebHistory，配合 GitHub Pages 404 redirect），21 條路由
...
| `chatEngine.js` | 對話引擎：串流回覆、群組、主動訊息、記憶總結、Heart Voice、生理期關心 |
