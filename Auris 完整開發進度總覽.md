...
**修法**：
- 新增 `services/keyboardViewport.js`：focus 前記錄目前頁面的基準框；鍵盤期間監聽 `visualViewport.resize/scroll`，只計算 visual viewport 遮住頁面的 `top/bottom inset`，套到當前 `.keyboard-page`，不改 `.phone`、不讀 `window.scrollY`、不用 transform 平移整個 App。事件以 double-rAF 合併，並在 60ms trailing reconcile 修正 iOS PWA 偶發的過早 `offsetTop`；blur 只排程複核，viewport 回升才清理。
...
- App 全域 smooth `scrollIntoView` 排除 `.keyboard-page`，匿名監聽器改具名並在 unmount 清除；一般表單維持既有行為，縮小回歸範圍。`globalStore.keyboardOffset`／BottomNav `kb-hidden`、`body position:fixed`、`.phone 100dvh` 均不動。

**驗證**：新增 `keyboardViewport.test.js` 12 案例（幾何、門檻、offsetTop、無效值、focus 前置狀態、blur/refocus、送出 interaction lock、destroy 清理）；vitest 178/178、production build 成功。390×844 本地手機尺寸檢查：聊天室與貼文頁皆為單一捲動區，header 與輸入列正常貼齊；WebKit 黑塊／切片仍須以 iPhone PWA 的 dev 部署實機驗收，測試全綠不可取代此門檻。
...
| `services/__tests__/keyboardViewport.test.js` | 新增 12 案例 |
