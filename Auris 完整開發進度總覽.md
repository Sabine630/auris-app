...
**最後更新**: 2026-06-01
**當前版本**: P59（生理期關心、防誤刪角色、清空對話可選範圍、上線後連續修復）
**狀態**: 持續優化中
...
4. ✅ **自動生成觸發引擎** — P50（2026-05-25）
   ...
   - 防重複機制（日記查當日是否已存在）
5. ✅ **Bug 修復與 UX 優化** — P51（2026-05-25）
   - 修正全站時間顯示：「3時」→「3小時前」（7 個頁面）
   - 記憶抽屜新增「編輯內容」功能（鉛筆按鈕 → inline 編輯 title + content）
   - 禁止 iOS / Android PWA 左右邊緣滑動觸發瀏覽器返回手勢
6. ✅ **通知系統修復、onboarding 防重複、記憶手動新增** — P52（2026-05-25）
   - 修復通知系統：貼文／日記／夢境／心聲生成後補寫 `notifications` store（原本完全沒寫入）
   - 修復 onboarding 重複出現：已有角色但 `onboarding_done` 為空時自動補設並跳過
   - 記憶抽屜頭部新增「+」按鈕，可手動輸入標題（選填）與內容新增一筆記憶
7. ✅ **更新公告系統** — P53（2026-05-25）
...
#### 功能描述
有使用者在聊天列表左划時，把「刪除」誤解成「刪除這串對話」，點下去後**角色連同日記/夢境/貼文整個消失**，造成嚴重資料損失。本次從互動設計面根治。

**問題背景（資料結構）：**
- 本 app「一個角色 = 一段對話」，無「同角色多段對話」概念。messages 與 diary/dreams/moments 全部僅以 `charId` 索引，綁的是角色而非單次對話。
- 左划三顆按鈕中「清空」與「刪除」icon 都是垃圾桶、僅差兩條線，視覺難分；且「刪除」實際會連角色一起殺，違反所有 IM（LINE/iMessage）左划「刪除＝只刪對話串」的共識。

**修復內容：**
- **左划移除「刪除角色」入口**：聊天列表左划只剩「置頂 / 清空」，永遠不會在此處殺到角色。刪除角色統一改走「設定 → 角色管理」頁（該頁已有含確認 modal 的刪除功能）。移除 `deleteChar` 函式。
- **「清空」改為自訂確認 modal（含勾選框）**：取代原本資訊不足的 native `confirm()`。modal 明確標示「角色不會被刪除」，並提供勾選框「同時清除日記、夢境、貼文」，**預設不勾**——只清聊天訊息與對話記憶，避免「我只想清訊息結果日記也沒了」的二次誤刪；想徹底重新開始的使用者可主動勾選連帶清除。
- **通知連動避免死連結**：心聲（`hv`）內容實際存於 `memories` store，基本清空一律會刪它，故 `hv` 通知**永遠**跟著清空一併清除（否則清空後通知還在、點進去卻是空的）；`post/diary/dream` 通知則僅在勾選連帶清除時清。
- **批次清空共用同一 modal**：管理模式的「清空記錄」也走新 modal，支援連帶清除選項。批次「刪除角色」保留（藏於二層選單、需多選＋確認，非誤刪來源）。

#### 受影響檔案

| 檔案 | 變動說明 |
|------|----------|
| `auris-vue/src/views/ChatListView.vue` | 移除左划刪除按鈕與 `deleteChar`；新增清空確認 modal（含連帶清除勾選框）；`clearChat`/`batchClear` 改走 `openClearModal` + `confirmClear` |
| `auris-vue/src/views/SettingsView.vue` | 版號更新至 P58 |
