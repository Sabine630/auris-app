...
本檔只記錄已完成版本的時間線，不再重複維護待辦清單。尚未完成、待驗證與暫緩項目的唯一來源：[`docs/ROADMAP.md`](docs/ROADMAP.md)。

已上線的使用者能力則以 [`product_feature_list.md`](product_feature_list.md) 為準。
...
### P109 分享卡支援整段連發（2026-07-12）

**背景**：實測回報——角色一次回覆常被 splitReply 切成連發多顆泡泡，分享卡只帶長按那一顆，他的話被攔腰截斷、圖看起來不連貫。

**修法**：
- `ChatRoomView` 新增 `msgGroupOf(m)`：往前後收集**相鄰且同角色**的文字泡泡（碰到對方訊息／心聲／輕觸行／圖片即斷開）＝整段連發；開分享卡預設整段帶上，多於 1 則時顯示勾選「帶上整段連發（N 則，保持完整）」可退回單則。「帶上你/他說的上一句」照舊（找對方訊息，不受連發影響）。
- `shareCard.js`：同角色相鄰泡泡間距 36→14 收緊成同一段話，且只有連發的**最後一顆**做小圓角尾巴（比照聊天室泡泡群組視覺）。

**驗證**：vitest 87/87；build 成功。

| 檔案 | 變更 |
|------|------|
| `views/ChatRoomView.vue` | `msgGroupOf` 連發收集＋「帶上整段連發」勾選（預設開） |
| `services/shareCard.js` | 連發間距收緊＋尾巴圓角只給末顆 |

---

### P110 全域確認彈窗吃不到主題修正（2026-07-12）

**背景**：實測回報「有些彈窗沒有吃到主題」——全域確認彈窗（`window.confirm_`，刪心聲／清聊天等全站共用）在深色等主題下一直是白底。

**原因**：與 P107 分享卡同族——主題變數只掛在 `#phone-container [data-theme]`，而確認彈窗是 `Teleport to="body"` 渲染在容器**外**，CSS 變數解析不到主題覆寫、一律落回 `:root` 預設奶白。全站唯一的 Teleport 就是它，所以只有這顆彈窗異常。

**修法**：`App.vue` 的 `syncRootBg` watcher（主題變更時同步 html 底色）順路把 `data-theme` **鏡射到 `body`**（`watch immediate: true`），任何傳送到 body 的元件自動命中 `[data-theme]` 選擇器。ARCHITECTURE 維護注意事項第 4/5 條同步補充。

**驗證**：vitest 87/87；build 成功。

| 檔案 | 變更 |
|------|------|
| `App.vue` | `syncRootBg` 加 body `data-theme` 鏡射＋watch immediate |

---

### P111 回憶頁擴建批——D1 回憶月報＋D3 時間膠囊（2026-07-12）

**背景**：維運與新功能討論定案的 D 組「關係有累積」特色線，D2 收藏盒已於 P106 上線，本批接續共用載體「我們的回憶」頁連續施工 D1＋D3。

**D1 回憶月報**：每月首次開 app 背景自動為每個角色生成上個月的「我們的這個月」＋通知（點通知直達回顧頁）。生成門檻**月訊息 ≥ 100**（排除心聲，不到靜默跳過）。素材＝該月自動總結至多 10 條（超量取最新）→ 無總結 fallback 該月頭尾各 20 則原文（單則截 60 字）→ 再不足只出統計層（不打 API）。統計全本地零 token：訊息數／聊天天數／常聊時段（`dayPeriod`）／心情打卡分佈（`mood_log`）／相識・在一起紀念日里程碑。信 150–250 字角色口吻；金句摘錄取該月收藏（角色說的優先、至多 3 則）。存 settings `monthly_reviews`（同月重生覆蓋）。「我們的回憶」頁改**三分頁（歷月回顧｜收藏｜寫給未來）**，回顧分頁可手動「生成本月回顧（搶先版）」與「補生成上月回顧」；**存成圖片**走 shareCard 新增的 `renderMonthCard` 月報卡面（P106 F1 預留的共用引擎）。自動生成標記 `monthly_review_run_{YM}` 全部成功才寫，API 失敗下次開 app 重試。

**D3 時間膠囊**：「寫給未來」分頁寫一段話封存，開啟日 3 個月／半年／一年／自訂（**最短 30 天**，UI＋service 雙防線）。「他也寫一封」**預設勾選**——埋的當下用完整聊天脈絡（`buildAIChatSetup`）生成角色的信、立即封存不顯示。到期走既有背景派發軌道（`runAllProactive` 新增 `runCapsuleDue`，比照定時提醒：不吃 min-gap、吃勿擾時段與防堆疊閘門、每輪最多一顆、失敗重試 3 次放棄通知但不影響手動拆）→ 角色主動訊息（kind `capsule`，可輕引使用者當時寫的話但不貼全文）＋通知。膠囊頁到期顯示「拆開膠囊」→ 兩封信並排展示；**拆封當天**兩封信注入對話易變段 context（chatEngine 直讀 settings 避免循環依賴），開聊天室能自然聊起。存 settings `capsules`。

**驗證**：vitest 109/109（新增 capsules 12＋reviewEngine 8 共 20 測試）；build 成功；Playwright 實測 9 張截圖——三分頁渲染、門檻 toast、埋膠囊（不勾角色信）、到期拆封並排、月報卡下載內容皆正確；通知 `review` type 導向 `/memories/:id?tab=review` 驗證通過。

| 檔案 | 變更 |
|------|------|
| `services/reviewEngine.js` | 新增：月份工具、`computeStats`／`pickMaterial`（純函式）、`generateMonthlyReview`、`runMonthlyReviews` |
| `services/capsules.js` | 新增：膠囊 CRUD＋`dueCapsules`／`markDueResult`／`openCapsule`（30 天下限、未到期不可拆防線） |
| `services/chatEngine.js` | 新增 `generateCapsuleLetter`／`generateCapsuleDueMessage`；`buildAIChatSetup` 易變段注入拆封當天膠囊；`PROACTIVE_KINDS` 加 `capsule` |
| `services/shareCard.js` | 新增 `renderMonthCard` 月報卡面（統計列＋回顧信＋金句，主題配色與浮水印同對話卡） |
| `views/MemoriesView.vue` | 雙→三分頁改造：歷月回顧卡（統計/里程碑/信/金句/存圖/手動生成）＋寫給未來（表單/封存倒數/拆封並排/刪除） |
| `App.vue` | 掛 `runMonthlyReviews()`（開 app 背景）＋`runCapsuleDue()`（併入 runAllProactive 派發鏈） |
| `views/NotificationsView.vue` | `review` 通知導向回顧分頁 |
| `__tests__/capsules.test.js`、`__tests__/reviewEngine.test.js` | 新增 20 測試 |

---

### P112 我們的默契——D4 專屬默契自動累積（2026-07-15）

**背景**：D 組「關係有累積」特色線最後一項。自動總結記憶抓的是「事件與資訊」，D4 抓的是「相處方式」——口頭禪、專屬稱呼、暗號、共同的梗，注入 prompt 讓角色自然使用，成為長期相處的累積感來源。

**修法**：
- **搭自動總結同一次呼叫抽新梗（零額外 API 呼叫）**：`summarizeToMemory` 的 system prompt 追加指示——把現有默契清單給模型、只回「不在上列」的新增項（模型自行去重），摘要後另起一行輸出 `BONDS: ["…"]`（每條 20 字內、最多 3 條、只收明確重複或約定過的）。`parseSummaryBonds`（純函式）解析，格式壞掉一律當沒有新梗、摘要不受影響；已滿上限就不問（省 token）。
- **上限 15 條**（`BOND_CAP`）：滿了新梗靜默不收，UI 顯示「整理一下才能收新的」提示。`mergeBonds`（純函式）合併：完全相同文字去重、單條截 40 字、寫前重讀最新角色物件（沿用 maybeAutoSummarize 防撞法，只覆蓋 `bonds` 欄位）。**新梗默默入列、不提示**（定案）。
- **注入 system prompt 穩定段尾**：`buildAIChatSetup` 組 `bondsCtx`（僅 enabled 條目）——變動頻率低（只在總結/手動編輯時變），放穩定段不破壞快取價值；措辭強調「合適時機自然使用、不要逐條展示」。
- **記憶抽屜加「記憶｜我們的默契」分頁**：默契條目可看/改/刪/手動新增/單條開關（比照記憶條目 UI），footer 顯示 `N/15 條・已開啟的會自然融入他的說話方式`；開抽屜時從 DB 重讀（背景總結可能剛默默補了新條目）。
- `characters` 軟欄位 `bonds: [{ id, text, enabled, createdAt }]`，免 DB 升版，舊角色無此欄位視同空陣列。

**驗證**：vitest 116/116（新增 parseSummaryBonds 4＋mergeBonds 3 共 7 測試）；build 成功；Playwright 實測——默契分頁渲染、開關狀態、手動新增並驗證落庫（4 條含 enabled 狀態全對）。

| 檔案 | 變更 |
|------|------|
| `services/chatEngine.js` | `BOND_CAP`／`parseSummaryBonds`／`mergeBonds`；`summarizeToMemory` 同呼叫抽梗＋寫前重讀合併；`buildAIChatSetup` 穩定段注入 `bondsCtx` |
| `views/ChatRoomView.vue` | 記憶抽屜「記憶｜我們的默契」分頁＋默契 CRUD（`persistBonds` 只覆蓋 bonds 欄位） |
| `__tests__/chatEngine.test.js` | 新增 7 測試 |

---

### P113 資安修復批——備份匯入原子化＋API 設定不隨備份走＋外部圖片過濾（2026-07-15）

**背景**：2026-07-15 資安審查報告（《Auris 專案健檢與資安審查報告 2026-07-15.md》）發現 1 項高優先資料完整性問題與多項中優先問題，本版修復其中與匯入路徑相關的全部項目。

**修法**：
- **匯入原子化（高優先）**：`importAllData` 原本「清空→還原」分散在數百個獨立 transaction，中途失敗（quota、I/O）無法回滾，且備份缺某 store 會被靜默清空。改為：(1) 新增 `REQUIRED_STORES`——v1 備份格式自誕生起必含的 11 個 store，缺任一直接拒絕匯入（`chat_memories`/`wishes`/`notes` 是備份功能之後才新增的，允許缺席、視為空）；(2)「清空→還原→補回本機 API 設定」收進**單一 multi-store readwrite transaction**，任一筆寫入失敗整批 abort、IndexedDB 自動回滾；排隊時的同步錯誤（無效 key）也主動 `tx.abort()`。
- **API 設定屬本機、不隨備份移動（中優先）**：新增 `LOCAL_ONLY_SETTINGS`（`api_key`/`api_provider`/`api_base`/`api_model`）——匯出不寫入（原本只排除 api_key）、匯入一律忽略並保留本機原值。堵住「分享的備份檔夾帶惡意 `api_base`，匯入後本機金鑰被送往攻擊者伺服器」的攻擊路徑。
- **匯入圖片過濾外部 URL（中優先）**：新增 `stripUnsafeImage`——訊息 `image` 欄位只接受 `data:image/` 內嵌圖，外部 URL 一律移除（防追蹤像素洩漏 IP 與上線時間）。套用於三條匯入路徑：全站備份、單角色匯入、聊天記錄 JSON 匯入。角色頭像原本就有 `startsWith('data:')` 渲染防護，不需改。
- **匯入失敗訊息**：SettingsView 改顯示實際原因（格式錯誤／quota 等），並註明「原有資料未受影響」（原子化後此保證成立）。

**驗證**：vitest 130/130（新增 `db.test.js` 14 測試，跑在 fake-indexeddb 上——含「寫入中途失敗→整批回滾」「惡意 api_base 被忽略」「外部圖片被過濾」「匯出→匯入 round-trip」）；build 成功。

| 檔案 | 變更 |
|------|------|
| `services/db.js` | `REQUIRED_STORES`／`LOCAL_ONLY_SETTINGS`／`stripUnsafeImage`；`importAllData` 改單一 transaction；`exportAllData` 排除全部 API 設定；`importCharacterData` 過濾圖片 |
| `views/ChatRoomView.vue` | `importChat` 過濾外部圖片 URL |
| `views/SettingsView.vue` | 匯入失敗訊息顯示實際原因 |
| `services/__tests__/db.test.js` | 新增（14 測試，fake-indexeddb） |
| `package.json` | devDependency 新增 `fake-indexeddb` |

---

### P114 資安強化批——Vertex token 身分隔離＋診斷去敏＋主題預讀修復（2026-07-15）

**背景**：P113 復驗仍有兩項可由本機程式碼直接修復的安全問題：Vertex OAuth token cache 沒綁 service account 身分，以及診斷 ring buffer 可能保存第三方原始錯誤文字；另有首頁主題預讀仍指向淘汰 DB 名稱的工程缺口。

**修法**：
- **Vertex token cache 身分隔離**：cache identity 改用 `project_id + client_email + private key SHA-256 指紋`，只保存不可逆指紋；不同帳號、不同 project 或同帳號換 key 都不共用 token。有效期改採 OAuth `expires_in`，並預留最多 60 秒／10% 安全緩衝，不再固定 3,500,000 ms；失敗訊息只保留錯誤碼，不串接完整第三方 response。
- **診斷信任分級（strict／trusted-local）**：ring buffer 改為 schema 2 結構化欄位（`code`／`status`／`provider`／`model`／`location`／`localMessage`），匯出時才組人類可讀文字。**strict（預設）**——LLM／網路／`unhandledrejection` 等來源不明錯誤只保存分類與安全 metadata，絕不保存原始 message（供應商可能回顯使用者輸入）；**trusted-local**——只有受控 call site（window runtime error 且 `new URL(filename).origin === location.origin` 驗證同源、DB 初始化）明確指定才保留錯誤訊息（CSP 允許 vercel.live 等第三方 script，「有 filename」不等於本地程式），寫入前必經「**先刪控制字元**（換行切開的金鑰片段會重新黏合，才能被遮蔽）→ 遮蔽金鑰成 `[REDACTED]`（g flag 實際 replace）→ 遮蔽 URL 成 `[URL]` → 截斷 300 字」管線，保住本地 debug 資訊（P103 教訓）又不外洩敏感內容。`getErrors` 讀出時逐筆重新驗證型別、allowlist 並重跑遮蔽，限最新 30 筆、timestamp 解析後重輸出標準 ISO、location 剝除 query/fragment（localStorage 可被竄改，schema 標記不代表內容可信）；舊版字串資料一律 strict 重分類；`formatDiagError` 輸出前再做最後一道 allowlist／遮蔽。`exportDiag` 的 theme／provider／model 等 settings 值過 `safeLabel` 後才輸出。
- **主題預讀修復**：`index.html` 從正確的 `auris` DB 讀 theme；DB 不存在時 abort，避免預讀腳本建立沒有 stores 的空 DB；讀取完成關閉連線。SettingsView 每次切換主題同步 `auris-theme` localStorage，後續載入可在 Vue／IndexedDB 前立即套色。
- **工程防線修復（復查發現）**：(1) 三支 hook 原本只比對連續字串 `git push`／`git commit`，`git -C <repo> push origin main` 可完全繞過（實測確認）——新增 `scripts/hooks/lib.sh` 共用判斷，支援 `-C`／`-c`／`--long`／短旗標等 git 全域選項，並以臨時 git repo 補 14 條行為測試（`hooks.test.js`，標準與 `-C` 形式都要觸發、逃生口與負案例放行）。(2) `check-project-config.mjs` 新增：文字程式檔（js/vue/md/sh 等）含 NUL 位元組直接失敗——NUL 會讓 Git 視為 binary，diff 與金鑰掃描全部失效，等於在防線上開盲點。

**驗證**：新增 `api.test.js` 4 案例（同帳號快取、不同帳號隔離、換 key 失效、依 `expires_in` 到期重取）；`diag.test.js` 重寫為 25 案例（LLM 原文不保存、本地 TypeError 保留清理後訊息、金鑰遮蔽、換行拆金鑰黏合後遮蔽、URL query 遮蔽、300 字截斷、竄改 schema 2 資料重遮蔽＋超量限 30 筆＋timestamp 注入＋location 殘留、同源／vercel.live 跨源判定、`formatDiagError` 最後防線、匯出去敏等）；`hooks.test.js` 新增 14 案例（含 `git -C`、`--git-dir <path>` 分離形式繞過案例）；vitest 166/166 通過。

| 檔案 | 變更 |
|------|------|
| `services/api.js` | Vertex cache identity 加入 service account／key 指紋；有效期改用 `expires_in` |
| `services/diag.js` | 原始錯誤文字改安全分類；舊 ring buffer 讀取時去敏 |
| `services/llm.js`、`main.js` | 改傳結構化安全 metadata／本地錯誤碼 |
| `views/SettingsView.vue`、`index.html` | 診斷文案與主題 localStorage／IndexedDB 預讀修復 |
| `scripts/hooks/lib.sh`（新增）＋三支 hook | git 全域選項判斷，堵 `git -C` 繞過 |
| `scripts/check-project-config.mjs` | 文字程式檔含 NUL 直接失敗 |
| `services/__tests__/api.test.js`、`diag.test.js`、`hooks.test.js` | token 隔離、診斷信任分級與 hook 行為測試 |

---

### P115 iOS PWA 鍵盤輸入頁穩定化（2026-07-16）

**背景**：iPhone PWA 實機錄影顯示，聊天室與貼文留言在鍵盤升降期間會出現 header 被推離、黑色切片與畫面錯位。兩頁（連同群聊）共同使用「可捲動 `.page`＋內層第二捲動區＋sticky/blur 輸入列」，又被 App 全域延遲 300ms 的 smooth `scrollIntoView` 命中；Safari 鍵盤只縮 visual viewport，sticky 仍依 layout viewport 定位，形成捲動與合成層衝突。P83 曾把整個 `.phone` 綁 `--vvh/--vvtop` 平移而在實機改爆，P85 已回退，本次明確不重走該路徑。

**修法**：
- 新增 `services/keyboardViewport.js`：focus 前同時記錄頁面基準框與 resting `visualViewport.offsetTop`；鍵盤期間先把 viewport 位移換算成頁面局部座標，再計算 `.keyboard-page` 的 `top/bottom inset`。此換算避免把 `.phone` 的 safe-area 起點與 layout viewport 原點混用，造成 header／輸入列整體上移。不改 `.phone`、不讀 `window.scrollY`、不用 transform 平移整個 App。事件以 double-rAF 合併，並在 60／240／500ms 做有限 trailing reconcile，吸收 standalone PWA 延遲更新的 `offsetTop/height`；blur 只排程複核，viewport 回升才清理。
- 一對一聊天室、群聊、貼文留言統一為「header＋唯一 `.keyboard-scroll`＋普通 flex `.keyboard-input-bar`」；外層不捲動，header／輸入列移除 sticky。鍵盤 focus 時停用相關 backdrop blur 與頁面 transform/transition，降低 WebKit 合成層閃爍。
- 輸入列 pointer 操作期間鎖住版面，讓 textarea blur 後的送出 click 先完成，避免按鈕位移造成要按兩次；鍵盤開啟時移除 Home Indicator safe-area 額外 padding，避免鍵盤上方空帶。
- App 全域 smooth `scrollIntoView` 排除 `.keyboard-page`，匿名監聽器改具名並在 unmount 清除；route fade 與 active `.page` 移除水平 transform，只保留 opacity，避免鍵盤升降後的 WebKit 合成層切片。一般表單維持既有行為；`globalStore.keyboardOffset`／BottomNav `kb-hidden`、`body position:fixed`、`.phone 100dvh` 均不動。
- **實機第二輪根因校正**：23:28 錄影顯示鍵盤、header 與輸入列幾何已正常，但 AI 等待期間畫面仍以約 `.8s` 節奏在完整／缺塊間反覆；單聊與群聊當時都共用 `.tdot` 的無限 `transform:translateY()` 動畫。這符合 WebKit transform 動畫／shared backing repaint 範圍錯誤的已知型態。為避免小動畫把整頁升成錯誤合成層，輸入中／生成中三點改為靜態明暗，單聊與群聊的串流游標也取消無限 opacity 動畫；同時清掉腦內基地生成點與錄音鈕剩餘的無限 CSS 動畫。狀態提示仍清楚可見，但常駐 PWA 畫面不再建立持續動畫合成層。

**驗證**：`keyboardViewport.test.js` 14 案例（含 safe-area 座標原點、iOS 26 resting offset、240ms 延遲更新、門檻、focus／blur／refocus、送出 interaction lock、destroy 清理）；`iosCompositor.test.js` 3 案例鎖住等待圓點、串流游標與常駐 PWA 畫面不得重新加入無限 animation／transform。vitest 183/183、production build 與專案設定檢查成功。本機載入後的實際 stylesheet 規則確認 `.tdot`／`.gen-dot` 只有靜態 opacity、無 keyframes。WebKit 合成結果仍以 iPhone PWA 的 dev 部署為正式放行門檻，測試全綠不可取代實機。

| 檔案 | 變更 |
|------|------|
| `services/keyboardViewport.js` | 新增局部 visual viewport 幾何與生命週期 controller |
| `views/ChatRoomView.vue`、`GroupRoomView.vue`、`PostDetailView.vue` | 共用單一捲動區與鍵盤 controller |
| `assets/main.css` | `.keyboard-page`／`.keyboard-scroll`／`.keyboard-input-bar`、kb-active/open 狀態 |
| `App.vue` | 鍵盤頁排除全域 smooth scroll；focus listener 可清理 |
| `services/__tests__/keyboardViewport.test.js` | 14 案例，含 safe-area 局部座標與延遲 reconcile 回歸測試 |
| `services/__tests__/iosCompositor.test.js` | 3 案例，防止常駐 PWA 畫面重新引入無限合成動畫 |

---

### P116 iOS PWA 鍵盤可證偽診斷版（2026-07-17）

**背景**：P115 三輪依序處理鍵盤頁結構、safe-area 座標與無限動畫，但 iPhone 實機仍是唯一能確認 WebKit 缺塊／閃爍的管道。既有測試只能鎖住各輪假設，不能證明動畫或座標就是根因；本輪停止追加永久修法，改成一次部署可切換多個嫌疑人的觀測版。

**診斷方式與可證偽預測**：
- `?kbdiag=1` 顯示 visual viewport 高度／offset、頁面範圍、上下 inset、baseline、focus、standalone/browser 狀態；面板按鈕可直接重載並組合實驗。
- 「游標」開關套用 `nofx=caret`，只隱藏鍵盤頁焦點輸入框 caret；若約 0.8 秒閃爍消失，才支持游標 repaint 假設。
- 「毛玻璃」開關套用 `nofx=blur`，暫停全站 backdrop filter；「串流」開關套用 `nofx=stream`，網路仍串流但畫面只在完成後一次顯示，用來分離 blur 與 DOM mutation。
- Paint／Layer 分別套 `contain: layout paint` 與 `translateZ(0)` 到 `.keyboard-scroll`；若只有其中一項消除缺塊，才支持 repaint／合成層污染假設。
- 診斷 PWA 的動態 manifest 保留當前 path＋query，安裝後仍可由面板切換組合。正常網址沒有任何診斷 class、面板或串流差異。

**驗證界線**：單元測試只證明 query 解析、切換連結與 P115 controller 回歸未壞，不代表任一嫌疑已定罪。必須用同一支 iPhone、同一操作，分別比較 Safari 分頁與 standalone PWA 的基準／單一開關錄影後，才能決定正式修法。

| 檔案 | 變更 |
|------|------|
| `services/keyboardDiagnostics.js` | 診斷參數、即時 viewport 面板、面板切換連結與 controller snapshot |
| `services/keyboardViewport.js` | 發布 baseline、鍵盤開啟／關閉的幾何快照 |
| `views/ChatRoomView.vue`、`GroupRoomView.vue` | `nofx=stream` 時停止逐 chunk DOM mutation，完成後一次顯示 |
| `assets/main.css` | caret／blur／paint／layer 四組隔離實驗與診斷面板樣式 |
| `App.vue`、`main.js` | 啟動診斷服務；診斷模式 manifest 保留 path＋query |
| `services/__tests__/keyboardDiagnostics.test.js` | query 組合、無診斷時惰性、面板切換 URL 測試 |

---
### P117 iOS PWA standalone 診斷辨識修正（2026-07-17）

**背景**：P116 診斷面板只用 `matchMedia('(display-mode: standalone)')` 判斷執行模式；iPhone 已由主畫面啟動、畫面沒有 Safari 網址列時，仍可能回報不符合該 media query，導致面板誤標為 `browser`，讓實機測試被錯誤退回重做。

**修法**：新增 `isStandaloneDisplay`，iOS 優先接受 `navigator.standalone === true`，並保留標準 `display-mode: standalone` media query 作為另一條判斷。這只修正面板標籤，不改鍵盤、viewport 或任何實驗效果。

**驗證**：新增 iOS `navigator.standalone` 與標準 media query 兩條單元測試；使用者原錄影應依新判斷重新顯示為 `standalone`。

| 檔案 | 變更 |
|------|------|
| `services/keyboardDiagnostics.js` | standalone 判斷同時支援 iOS 與標準訊號 |
| `services/__tests__/keyboardDiagnostics.test.js` | 兩條顯示模式辨識回歸測試 |

---
### P118 iOS PWA 純鍵盤架構對照實驗（2026-07-17）

**背景**：P116/P117 已排除 caret、backdrop filter、串流 DOM mutation，以及 `.keyboard-scroll` 的 paint/layer 隔離。使用者進一步確認 API 設定頁的自訂模型輸入框也會在鍵盤出現時缺塊；該頁不使用 `keyboardViewport.js`、`.keyboard-page` 或聊天串流，因此 controller 已被跨頁證據排除，嫌疑上移到 Auris 共用 shell 或 standalone WebKit。

**實驗**：新增同源靜態 `public/keyboard-lab.html`，不載入 Vue，不使用 `.phone`／`.screen`／`.page`、`body position:fixed`、absolute/fixed、blur、transform 或動畫；只保留普通 document flow、六個實色基準區塊、一個輸入框，以及事件驅動的 `visualViewport`／standalone 數值。P116 診斷面板新增「純頁測試」入口，從已安裝 PWA 內同源導向。

**可證偽預測**：
- 純頁不缺塊、Auris API／聊天會：支持 Auris 外層 shell（body fixed＋100dvh phone＋overflow-hidden screen＋absolute page）造成共享合成層問題。
- 純頁也缺塊：否定「Vue shell 是必要條件」，嫌疑上移到 standalone PWA／WebKit／viewport meta 或裝置環境。
- 此頁只用假輸入框，不讀寫 IndexedDB、API key 或任何使用者資料。

| 檔案 | 變更 |
|------|------|
| `public/keyboard-lab.html` | 新增完全脫離 Auris shell 的純鍵盤 A/B 控制頁 |
| `services/keyboardDiagnostics.js` | 黑色診斷面板新增「純頁測試」入口 |
| `services/__tests__/keyboardLab.test.js` | 鎖住控制頁不得引入 Vue shell與合成層 CSS，並確認 viewport 訊號 |

---
### P119 iOS PWA 共用 shell 分層對照（2026-07-17）

**證據更新**：使用者在同一支 iPhone、同一個 standalone PWA 與相同 viewport meta 下實測 P118 純頁，鍵盤開啟後六個基準區塊皆正常；Auris 聊天與 API 自訂模型輸入框則會缺塊。因此 standalone PWA／viewport meta 本身不足以觸發，Auris 共用外層 shell 已成為必要差異。

**實驗**：診斷面板新增 kbshell 單選列，一次部署提供四個可證偽模式：Body 只移除 body fixed、Page 只移除 page absolute、Clip 只解除 phone/screen overflow clipping、Flow 同時將手機 shell 改為普通 document flow；原殼可隨時回到基準。所有規則都綁在 kbdiag 產生的 html class，普通網址完全不套用。

**判讀方式**：
- 單一 Body／Page／Clip 正常：可定位到對應層。
- 三個單項皆異常、只有 Flow 正常：代表多層交互作用，長期修法應是手機版普通 flow，而非再疊單點 workaround。
- Flow 也異常：共用 shell 仍有未納入的差異，需再查 Vue mount／固定診斷面板或全域 focus scroll。

| 檔案 | 變更 |
|------|------|
| services/keyboardDiagnostics.js | 解析、顯示並切換 kbshell=body/page/clip/flow |
| assets/main.css | 新增只在診斷 class 下生效的四組 shell A/B 規則 |
| services/__tests__/keyboardDiagnostics.test.js | query 保留、模式白名單與回復原殼測試 |

---
### P120 iOS PWA fixed body 缺塊修復候選（2026-07-17）

**實機定層結果**：同一個 API 自訂模型輸入框依序測試 P119 四模式，Body 不缺塊、Page 缺塊、Clip 缺塊、Flow 不缺塊。Body 是唯一只改一個條件就恢復正常的模式；Flow 也包含同一項變更。因此 page absolute 與 phone/screen overflow clipping 均被排除，手機版 body position:fixed 是目前被實機 A/B 定罪的必要觸發條件。

**正式候選修法**：手機版與 standalone 共用規則將 body 改為 position:static，同時保留 height:100dvh、overflow:hidden 與 phone/screen 既有結構，避免把 P119 的完整 Flow 實驗誤當正式重構。診斷面板縮成「新版／舊Fixed」反向 A/B；只有明確選擇舊Fixed 才恢復已證實會缺塊的舊行為。

**規則同步**：AGENTS.md 與 CLAUDE.md 的舊原則已改為禁止手機 body fixed；新增 source regression test 鎖住正式 CSS 為 static，並確認 fixed 僅存在於反向診斷 class。實機放行仍需確認正常模式的 API 輸入與聊天室鍵盤期間皆不缺塊，且沒有 viewport 偷捲或底部空帶。

| 檔案 | 變更 |
|------|------|
| assets/main.css | 手機／standalone body fixed 改 static；保留舊 Fixed 反向診斷 |
| services/keyboardDiagnostics.js | shell 控制縮為新版與舊Fixed |
| services/__tests__/keyboardDiagnostics.test.js、keyboardShellFix.test.js | query 與正式 CSS 回歸測試 |
| AGENTS.md、CLAUDE.md | 更新 iOS PWA 鍵盤處理原則 |

---
### P121 iOS PWA body computed 證據強化（2026-07-17）

**P120 影片結果**：直接解碼 38.24 秒、1180×2556 HEVC 原始錄影，排除瀏覽器硬體影片圖層的灰色假截圖。「新版／shell original」在 API 自訂模型鍵盤期間出現約 1.88 秒白色缺塊，聊天室出現約 0.98 秒；同片「舊Fixed／shell fixed」沒有持續缺塊。這與 P119 實機結果衝突，但影片中的 shell 字樣只是 query 狀態，尚不能證明實際載入的 CSS。

**證據強化**：不再改動 shell CSS。`kbdiag` 即時面板直接顯示 `APP_VERSION`、`getComputedStyle(document.body).position`、body rect 的 top/bottom 與 `body.scrollTop`，下一支實機錄影可同時證明 P121、shell query 與 body 真實 computed 狀態。若新版顯示 `body pos static` 仍缺塊，再比對 P119 Body 與 P120 正式 CSS；若仍為 `fixed`，優先查 PWA 快取與舊 hashed asset。

| 檔案 | 變更 |
|------|------|
| `services/keyboardDiagnostics.js` | 面板新增 App 版本、body computed position、rect 與 scrollTop |
| `services/__tests__/keyboardDiagnostics.test.js` | 新增診斷證據輸出回歸測試 |
| `version.js` | 版號升為 P121，讓實機面板與部署版本可直接對照 |
| `ARCHITECTURE.md`、`Auris 完整開發進度總覽.md` | 記錄 P120 影片反證與 P121 判讀門檻 |

---
### P122 iOS PWA Absolute body 雙問題 A/B（2026-07-17）

**P121 回歸證據**：23.02 秒實機錄影明確顯示 `P121 · standalone`、`body pos static`、`shell original`，排除快取與 query 假象。鍵盤上方白帶在聊天室持續約 0.80 秒、貼文留言約 0.98 秒、API 自訂模型約 1.16 秒。API 鍵盤開啟時，body rect 由 `0…852` 變成 `-404…389`，表示 body static 雖避開 fixed 合成層的頂部缺塊，卻重新放行 iOS document 偷捲，使 App 底部離開可視區而露出白色底層。

**受控實驗**：不改正常網址的 Static 預設。`kbdiag` shell 列新增 `Absolute`，只在 `kbshell=absolute` 時將 body 設為 `position:absolute; inset:0; height:100dvh; overflow:hidden`，用來測試「不進入 fixed 合成層，又不讓 body 參與普通 document flow」能否同時消除頂部缺塊與底部白帶。面板同步新增 `window.scrollY` 與 `documentElement.scrollTop`，避免 `body.scrollTop=0` 掩蓋真正的 document 位移。

**放行門檻**：Absolute 必須在聊天室、貼文留言、API 三頁同時滿足頂部不缺塊、鍵盤上方無白帶、收起不跳位、body rect 不變負值。未通過前不得變更正式 Static CSS，也不得推 `main`。

| 檔案 | 變更 |
|------|------|
| `services/keyboardDiagnostics.js` | 新增 Absolute shell 白名單／按鈕，顯示 window 與 html scroll |
| `assets/main.css` | 新增只屬於 `diag-kbshell-absolute` 的 body absolute 候選規則 |
| `services/__tests__/keyboardDiagnostics.test.js`、`keyboardShellFix.test.js` | 鎖住 Absolute query、捲動證據與診斷隔離 |
| `version.js` | 版號升為 P122 |

---
### P123 iOS PWA Root Scroll Guard A/B（2026-07-17）

**P122 實機結論**：20.57 秒實機錄影確認 `P122 · standalone`、`body pos absolute`、`shell absolute`。聊天室、貼文留言、API 自訂模型三頁的鍵盤開啟過渡都顯示 `window.scrollY` 與 `documentElement.scrollTop` 從 `0` 跳為 `404`，body rect 同步從 `0…852` 變成 `-404…389`；visual viewport 底部為 `448`，body 底部只到 `389`，相差的 59px 正是影片中的白帶。Absolute 仍屬 document 座標，因此與 Static 一樣會被 iOS focus scroll 搬走，不能成為正式修法。

**受控實驗**：正式 Static CSS 維持不變。`kbdiag` 新增獨立 `kbroot=guard`／「Root鎖定」模式，只在文字輸入框聚焦且 visual viewport 比基準縮小超過 80px 時啟動；監聽 window scroll 與 visualViewport resize/scroll，把 `window`、`documentElement`、body 的 root scroll 歸零，並以 rAF＋60／240／500ms 有限複核覆蓋鍵盤動畫，不使用永久 polling。一般頁面捲動、鍵盤關閉與失焦狀態都不介入。

**可證偽讀值**：面板新增 `root guard/free`、累計修正次數與最後 `before→after`。實機需選 Static＋Root鎖定；通過條件為三頁鍵盤期間 `fix` 增加、`guard 404→0`、window/html scroll 最終為 0、body rect 不再為負值，且頂部無缺塊、底部無白帶、收鍵盤不跳位。未通過前仍不改正式 CSS、不推 main。

| 檔案 | 變更 |
|------|------|
| `services/keyboardDiagnostics.js` | 新增診斷限定 Root Scroll Guard、query／按鈕與 before→after 證據 |
| `services/__tests__/keyboardDiagnostics.test.js` | 驗證只在文字 focus＋鍵盤 viewport 開啟時歸零 root scroll，失焦與清理後不介入 |
| `assets/main.css` | 診斷面板新增 Root 控制列樣式，不變更正式 shell CSS |
| `version.js` | 版號升為 P123 |

---
### P124 iOS PWA 鍵盤雙缺塊正式修復（2026-07-17）

> 完整實機矩陣、雙根因、被否決方案與未來排障順序：[`docs/iOS PWA 鍵盤缺塊事後報告 P116-P124.md`](docs/iOS%20PWA%20鍵盤缺塊事後報告%20P116-P124.md)

**P123 實機放行**：直接解碼 43.612 秒、1180×2556、約 60fps 的 iPhone standalone PWA 原始錄影，逐秒總覽後再以 0.2 秒間隔檢查鍵盤開合。影片全程顯示 `P123 · standalone`、`body pos static`、`shell original` 與 Root鎖定。聊天室與貼文留言在鍵盤開啟時均由 `guard 404.0→0.0`，API 自訂模型輸入框兩輪分別為 `11.0→0.0`、`2.0→0.0`；三頁最終皆維持 `scroll w 0.0 · html 0.0`、body rect `0…852`。鍵盤收起後 visual viewport 回到 852、scroll 仍為 0，影片未見頂部缺塊、底部白帶或殘留跳位，P123 A/B 正式通過。

**正式化**：新增獨立 `keyboardRootScrollGuard.js`，由 `main.js` 啟動時自動安裝，不再依賴 `kbdiag` 或 `kbroot=guard`。正式入口同時要求 iOS/iPadOS 裝置與 standalone display；Safari 一般分頁、桌面 standalone、沒有 visualViewport 的環境完全不掛監聽。Guard 仍只在文字型 input／textarea／contenteditable 聚焦且 viewport 比 resting baseline 縮小超過 80px 時介入；checkbox 等非文字 input、鍵盤關閉、一般捲動與失焦均不修正。保留立即＋rAF＋60／240／500ms 有限複核，不使用永久 polling。

**診斷收斂**：移除 Root自由／Root鎖定按鈕與 `kbroot` query 生成；舊 P123 query 會被診斷連結清掉，不能關閉正式 Guard。`kbdiag` 只保留唯讀 `root official/off`、fix 與 before→after 證據，其他 compositor/shell 實驗仍可用。手機 `body static`、100dvh、overflow hidden、`.phone/.screen/.page` 結構與 P115 keyboard controller 均未改動。

| 檔案 | 變更 |
|------|------|
| `services/keyboardRootScrollGuard.js` | 新增 iOS/iPadOS＋standalone eligibility、正式 Guard、狀態訂閱與完整 cleanup |
| `services/keyboardDiagnostics.js` | 移除 Root A/B query／按鈕；改為唯讀顯示正式 Guard 狀態 |
| `main.js` | App 啟動即安裝正式 iOS standalone Root Scroll Guard |
| `services/__tests__/keyboardRootScrollGuard.test.js` | 驗證正式接線、平台邊界、404→0、失焦停止、非文字 input 不介入與 listener cleanup |
| `services/__tests__/keyboardDiagnostics.test.js`、`assets/main.css` | 清除 P123 Root A/B 接線與樣式，保留正式狀態證據 |
| `AGENTS.md`、`CLAUDE.md` | 維護原則更新為 body static＋正式 Root Guard 成套保留 |
| `version.js`、`ARCHITECTURE.md`、本檔 | 版號與正式修復證據同步至 P124 |

---
### P125 日記輸出語言與中英夾雜修復（2026-07-17）

**問題定位**：實際日記出現「Loss of control」等無角色設定依據的英文短語。資料流檢查確認英文由模型直接生成並落庫；`DiaryDetailView` 與 `formatContent` 只做標題拆分、HTML escape 與換行，沒有翻譯或插入英文。根因在 `generateDiary` 只有寬鬆的「用繁體中文」一句，既未禁止模型為強調／文藝感自行 code-switch，也沒有讀取角色的 `lang`，導致所有角色日記一律寫死繁體中文。

**修復方式**：新增集中式日記語言規則，依角色的繁體中文、簡體中文、日文、韓文或英文設定產生強制指令；繁體中文角色明確禁止為強調、文藝感、旁白感或內心戲自行夾入英文及其他語言。保留三類合理例外：角色設定明確要求多語混用、不可翻譯的專有名詞／品牌／作品名／程式碼，以及忠實原文引述。日記 prompt 同步帶入補充角色設定，使真正設定為多語風格的角色可符合例外，而不是一律硬擋。

**回歸防線**：新增 content engine 測試，鎖住五種角色語言、舊角色缺欄位回退繁體中文、繁體中文禁止無依據英文、日文不再被寫死中文，以及補充設定的多語例外接線。沒有採用生成後刪除英文字的破壞性做法，避免誤傷正常專有名詞與引用。

| 檔案 | 變更 |
|------|------|
| `services/contentEngine.js` | 日記依角色語言建立強制輸出規則，禁止無設定依據的中英夾雜並保留合理例外 |
| `services/__tests__/contentEngine.test.js` | 新增 9 項語言規則與日記 prompt 接線測試 |
| `version.js`、`ARCHITECTURE.md`、本檔 | 版號與修復紀錄同步至 P125 |

---

### P126 教學示範模式內容同步 P106–P112 功能（2026-07-17）

**背景**：教學網站（`?demo=1` 示範模式，P102 建立）的畫面元件與正式 App 共用、自動同步，但**教學文案與示範資料**停在 P105 左右的功能狀態：回憶頁（P106 收藏盒、P111 月報＋時間膠囊、P112 默契入注入線）沒有專屬教學文案（只顯示 fallback 通用說明）、示範世界也沒 seed 任何回憶相關資料——demo 裡回憶頁一片空白，示範不出這批「關係有累積」特色功能。

**修法（純內容同步，不動任何 App 邏輯）**：
- **教學文案補齊（demoGuideContent.js）**：新增 `memories` 路由條目（介紹三分頁：歷月回顧｜收藏｜寫給未來）；聊天室條目補上長按選單的「收藏成回憶、做成分享卡」與記憶抽屜的「我們的默契」分頁；關係主頁條目補「我們的回憶」入口說明。
- **示範資料 seed（demoData.js）**：夜雨角色補 `bonds` 軟欄位 3 條默契；settings 新增 `keepsakes` 2 則收藏（對應 seed 訊息 m3、m9 的快照）、`capsules` 2 顆膠囊（一顆封存中含兩封信的等待感、一顆已拆開展示雙信並排）、`monthly_reviews` 上月月報 1 份（統計＋回顧短信＋金句）。
- **假回覆腳本（demoData.js `demoReply`）**：新增「時間膠囊」與「回顧短信」兩個關鍵字分支——demo 裡埋膠囊勾「他也寫一封」或生成月報時，回對應題材的假信件而非聊天語句。

**驗證**：vitest 216/216 通過；Playwright 實測 demo 模式 8 項全過——月報卡（統計＋回顧短信）、收藏 2 則、封存中／已拆膠囊卡、回憶頁教學面板文案、記憶抽屜默契分頁 3 條＋`3/15 條` footer。

| 檔案 | 變更 |
|------|------|
| `services/demoGuideContent.js` | 新增 `memories` 條目；`chat`／`relation` 條目補 P106–P112 功能說明 |
| `services/demoData.js` | 夜雨 `bonds` 3 條；seed `keepsakes`／`capsules`／`monthly_reviews`；`demoReply` 新增膠囊信與回顧短信分支 |

---

### P127 匯入圖片、JSON 資源與備份 Schema 安全強化（2026-07-18）

**背景**：P113 已完成全站備份的單一 multi-store transaction 與回滾，不需重做原子性；本批針對資安審查 HARD-02 的剩餘缺口補防線：匯入入口讀檔前沒有容量上限、`stripUnsafeImage()` 只看 `data:image/` 字首、v1 備份只有 store／keyPath 存在性而沒有核心欄位型別與資源總量驗證。

**共用匯入驗證層**：
- 新增 `services/importValidation.js`，三條入口統一在 `file.text()` 前檢查檔案大小：完整備份 64 MB、單角色 24 MB、聊天 32 MB；JSON 解析失敗回傳不含原始內容的安全錯誤。
- 圖片只接受 JPEG／PNG／WebP 的 `;base64,` data URL，同時核對宣告 MIME 與實際 magic bytes；單張解碼後上限 5 MB，並依匯入類型限制圖片總量。SVG、非 base64、偽造 MIME、損毀或超限圖片會在寫入 IndexedDB 前拒絕或移除；外部 URL 仍由 `stripUnsafeImage()` 移除，保留 P113 防追蹤像素行為。
- JSON 增加 store 筆數、總 records、單一文字 200,000 字、文字總量、陣列長度、物件欄位數與巢狀深度上限，避免超大或刻意深巢狀內容拖垮主執行緒。
- v1 schema validator 驗證每個 store 的 keyPath 為非空字串且不可重複，並檢查角色名稱、訊息角色／內容、各 charId／groupId、通知文字、時間戳與群組成員等核心欄位型別；整份備份全部通過後才進入 P113 既有原子 transaction。

**入口與回歸防線**：SettingsView 全站匯入、CharManageView 單角色匯入、ChatRoomView 聊天匯入全部改走共用讀檔／資源驗證；聊天不再逐筆靜默略過壞資料，而是整包先驗證、回傳具體且不洩漏內容的錯誤。新增測試覆蓋讀檔前阻擋、三種合法 raster、SVG／非 base64／偽造 MIME、圖片容量、文字／筆數上限、重複 key、核心欄位型別及合法匯入。

**驗證**：Vitest 23 個測試檔、226/226 通過（含目前教學沙盒完整資料匯出→匯入 round-trip 與 P113 transaction rollback）；production build、project config checks 通過。本機 `?demo=1` 實測完整備份／單角色／聊天三條入口的損毀 JSON 均在寫入前顯示安全錯誤，console 0 warning/error。

| 檔案 | 變更 |
|------|------|
| `services/importValidation.js` | 新增匯入檔案、圖片、JSON 資源與 v1 store schema 共用驗證 |
| `services/db.js` | 全站／單角色匯入接驗證層；`stripUnsafeImage` 升級為 MIME＋base64＋magic bytes＋容量檢查 |
| `views/SettingsView.vue`、`views/CharManageView.vue`、`views/ChatRoomView.vue` | 三條匯入入口在讀檔前套用各自檔案上限，統一安全錯誤訊息 |
| `services/__tests__/importValidation.test.js`、`db.test.js` | 新增資源／schema／圖片驗證與 round-trip／rollback 回歸測試 |
| `version.js`、`ARCHITECTURE.md`、`docs/ROADMAP.md`、資安審查報告、本檔 | P127 版號、架構、完成狀態與驗證紀錄同步 |
...
**修復**：
- hashtag 收尾邏輯抽成 `extractPostTags()`：不再用單一跨行 regex，改逐行從尾端往前收、每行以無回溯疑慮的 `/^#\w+$/` 判斷；行為與舊版完全一致（含「同一行多 tag 只抽行首那個」「中文 hashtag 因 `\w` 僅 ASCII 而不處理」等既有邊界）。
- ci.yml 加 `permissions: contents: read`（deploy.yml 原本就有宣告，未中）。

**驗證**：新增 4 個 `extractPostTags` 測試（行為一致性＋惡意長輸入毫秒級完成的防回歸），全套 Vitest 233/233 通過。CodeQL alert 需待修復隨下次發版進 main 後自動關閉。

| 檔案 | 變更 |
|------|------|
| `services/contentEngine.js` | hashtag 收尾改 `extractPostTags()` 逐行處理，移除 ReDoS regex |
| `.github/workflows/ci.yml` | 補 `permissions: contents: read` 最小權限 |
...
### P133 Vertex 空回應歸因與診斷補強（2026-07-29）
...
### P134 繁體正規化誤傷專有名詞、404 錯誤歸因、供應商錯誤代碼（2026-08-05）

三件互相獨立的實機回報，共通形狀都是「**訊息／輸出指向錯的地方**」。

**① 繁體正規化把人名地名改壞**

使用者打「格林格拉斯家的秘密」，角色回覆落庫後變成「格林**葛**拉斯家的秘密」，看起來像模型打錯字。實際上是 P132 加的 `normalizeCharacterOutput` 改的——OpenCC `twp`（台灣詞彙）是**詞組級**替換，詞表裡混了一批「短音譯碎片」條目（`格拉斯→葛拉斯`），只要碎片是某個更長專有名詞的一部分就會在內部誤觸發。本機實測 15 個名字**有 14 個被改壞**（道格拉斯→道葛拉斯、伊斯坦布爾→伊斯坦布林、拉莫斯→拉摩斯、加納利群島→迦納利群島…）。

修法演進（前兩案都被否決，記錄以免重蹈）：
- ~~改用字級 `tw`~~ → 會讓「網絡／信息／軟件／屏幕」等中國用語留著不轉，失去模組原本價值
- ~~兩段式（先 `tw`，無變化就略過）~~ → 「模型寫繁體字＋中國詞彙」時整段略過，同樣漏掉
- **採用**：沿用 `twp`，但濾掉 9 條會誤觸發的碎片條目（字典 774→765）。詞彙在地化幾乎零損失，`肖邦→蕭邦` 等正確譯名一律保留

**② 連線測試把所有 404 說成「網址錯了」**

`ApiView.testApi` 對 404 一律回「找不到這個 API 位址，請確認自訂網址」。但 Google 的 OpenAI 相容端點在**模型不存在**時同樣回 404、訊息含 `not found`，於是使用者被要求去改一個本來就正確的網址（甚至是 app 內建預設值）。改為只依三個**本地訊號**分流，不猜供應商措辭：回應是不是 JSON／錯誤訊息有沒有回述我方送出的 modelId／網址是誰決定的（`custom`／`default`／`app-built` 三態）。Vertex 的網址由程式自組且嵌有 `project_id`，404 可能是專案問題，故獨立為 `app-built`——沒點名模型就原樣回傳供應商訊息，不臆測。

**③ 診斷看不出供應商到底回了什麼**

`HTTP 404`／`HTTP 429` 分不出「模型不存在 vs 網址錯」「每分鐘上限 vs 每日配額」。新增 `pcode` 欄位記供應商自己的錯誤代碼。這是診斷的**第一個開放值欄位**（其餘皆封閉 allowlist），故採形狀過濾四道：不得含任何空白（自由文字必含空白）→ 僅 `[A-Za-z0-9_.\-/]`（**中文一字不通過**）→ 長度 1–64 → 金鑰特徵擋。**絕不讀 `error.message`**，候選鍵名清單裡結構性地沒有它。擷取用深度受限（≤4）的鍵名掃描而非寫死路徑，抓不到就空著、不會抓錯。

| 檔案 | 變更 |
|------|------|
| `services/zhPhraseBlocklist.js` | **新增**：`DROP_SOURCES`（9 條碎片）與 `filterPhraseDict()`；只走 `opencc-js/to/twp` 公開匯出、不碰套件內部 dict 檔；結構不符時原樣回傳（退回改版前行為，不讓轉換掛掉） |
| `services/zhTwWorker.js`、`services/outputLanguage.js` | 兩處建 converter 改套 `filterPhraseDict`，訊息協定與失敗退回原文的行為不變 |
| `services/connectionError.js` | **新增**：純函式 `describeConnectionFailure({ status, raw, data, modelId, baseKind })`；判斷順序與改版前一致（401→429→404→403） |
| `views/ApiView.vue` | 兩處失敗分支改用上述函式；新增 `isUsingCustomBase()`。**成功路徑一字未動** |
| `services/llm.js` | 新增 `extractPcode()`（鍵名優先序 `status→code→type→reason→errorType`——`code` 必須先於 `type`，OpenAI 的 `type` 是籠統桶、`code` 才是具體原因）；`httpError` 多帶 errObj，6 個呼叫點同步 |
| `services/diag.js` | 新增 `sanitizeCode()`（不共用 `safeLabel`——它的白名單沒有底線，會把 `model_not_found` 洗成 `modelnotfound`）；`pcode` 於寫入／讀出／輸出**三層**各清洗一次 |
| `Auris 維運速查.md` | 新增「五、使用者回報的常見處理」，含名字被改壞的症狀／成因／怎麼確認是我們改的／**怎麼修（加一個詞、補一條測試，不動邏輯）** |

**驗證**：全套 Vitest **702 綠**（新增 85 項）、production build 通過。繁體那項另以真實 OpenCC 字典端到端驗證：13 個曾被改壞的名字全數保住、`肖邦→蕭邦` 仍轉、11 個中國用語照常在地化。

**已知限制**：碎片排除清單只涵蓋已知的 9 條，清單外仍可能有其他碎片誤觸發——這是降低發生率，不是根治。新增方式已寫進維運速查。

---

### P135 重要日期常駐注入——角色不再自己編生日（2026-08-08）

**背景**：使用者問角色「你生日幾號」，角色堅稱是九月八號，但 app 裡設定的角色生日是 9 月 3 日。使用者把紀念日卡片貼給他看也沒用，角色仍回「不是。九月八號。」

**病灶不是模型固執，是它根本看不到這個欄位。** `getPersonalDateCtx()` 只在**當天**才注入提示：

```js
if (mmdd(char.birthday) === today) parts.push('今天是「' + char.name + '」的生日🎂');
```

其餘 364 天，模型完全不知道生日是哪天。而角色卡的系統 prompt 帶了年齡、職業、居住、個性、背景故事、近況、喜好、稱呼、口頭禪、關係背景……**就是沒有這四個日期**。模型被問到就自己編一個，那個錯誤答案又進了對話歷史，反而變成它「記得」的事實，一路堅持——使用者拿正確資料也糾正不回來。

受影響的四個欄位（都可在 UI 設定、都只在當天才進 prompt）：角色生日、使用者生日（`me_settings`）、相識紀念日、在一起紀念日。

**修法**：新增 `getKeyDatesCtx()`，把四個日期當成**常駐既定事實**注入角色卡模板（穩定段，吃得到 prompt cache）。原本的當天祝賀邏輯 `getPersonalDateCtx()` **保留不動**，兩段並存——一個是「今天是你生日🎂」，一個是「你的生日是 9 月 3 日」。

格式取捨（使用者指定）：**生日只給月日、不給年份**，年齡以角色卡既有的「年齡」欄位為準，避免兩者互相矛盾；相識日與在一起紀念日給完整日期，因為年份決定「在一起多久」且與年齡無關。文案含四個要素：必須正確回答／不可自行編造／**不要主動提起也不要反覆確認**（比照作息與天氣感的措辭，避免角色沒事就報生日）／年齡以基本資訊為準。

**順手修掉一個既有的當機路徑**：驗收時實測發現，日期欄位若不是字串（`{}`、數字、`Date` 物件…），`fmtMonthDay` 的 `.split` 與**P135 之前就存在的** `mmdd()` 的 `.slice` 都會拋 `TypeError`。兩者都在 `buildAIChatSetup` 的必經路徑上，一拋錯整個角色的聊天就不能用，使用者只會看到「錯誤：dateStr.split is not a function」。而 `importValidation.js`／`db.js` **並未對這三個日期欄位做型別驗證**，匯入的角色 JSON 或損壞資料就能觸發。改為先驗 `typeof` 與 `^\d{4}-\d{2}-\d{2}` 格式再取值，非法值一律回 `null`。

| 檔案 | 變更 |
|------|------|
| `services/chatEngine.js` | 新增 `getKeyDatesCtx()`、`fmtMonthDay()`、`fmtFullDate()`；`getPersonalDateCtx` 改 export（供測試迴歸鎖，行為未動）；`mmdd()` 補格式驗證；角色卡模板插入 `${keyDatesCtx}` |
| `services/__tests__/chatEngine.test.js` | 新增 38 項：四欄位組合、月日不補零、缺值不留空標題、四個約束字樣、當天 🎂 迴歸鎖、6 種垃圾輸入 × 兩個函式的不拋錯與不滲垃圾 |

**驗證**：全套 Vitest **740 綠**（新增 38 項）、production build 通過。另以 8 種垃圾輸入（物件／數字／陣列／`Date`／`null`／亂字串／含破折號字串／`2001/09/03` 格式）實測：全部不拋錯、輸出不含 `[object Object]`／`NaN`／`undefined`。

**已知限制**：救不回已經吵過的對話——模型先前講錯的日期仍在對話歷史裡，那幾則可能還會拉扯，最乾淨的做法是刪掉錯的訊息。另外對現有使用者是行為改變：角色會突然「知道」以前不知道的事。
...
而 Auris **沒有刪除單則訊息的功能**（長按選單只有表情反應／複製／收藏／分享，加上限最新一則的編輯重傳與重新生成）。使用者只剩三條路：等它滾出 prompt 視窗（預設 20 則）、對最新一則重新生成、或清空整個對話。都不理想。

**為什麼選編輯而不是刪除**：`chatEngine.js:664` 已註明 **Gemini／Anthropic 會拒絕連續同角色的訊息**。刪掉中間一則會讓前後兩則同角色相鄰、破壞 user/assistant 交替結構，得再補合併／補位邏輯。編輯則則數不變、角色順序不變、交替結構不變，只換文字——結構上完全安全。

另查證衍生資料的綁定方式：紀念品（`keepsakes`）存 `msgId` **＋ content 快照**，待續事件（`continuity`）存 `sourceMsgId` **＋ sourcePreview 快照**，兩者都不是即時讀原訊息，所以編輯不會弄壞它們，最多是「回到來源訊息」時文字與當初預覽略有出入。

**範圍（使用者界定）**：只做角色回覆，使用者訊息那側完全不動。

| 訊息 | 之前 | 之後 |
|---|---|---|
| 角色回覆・最新一則 | 只有「重新生成回覆」 | **編輯內容** ＋ 重新生成（兩個都給） |
| 角色回覆・往前的 | 無任何修改手段 | **編輯內容** |
| 使用者訊息・最新一則 | 編輯並重傳 | 不變 |

最新那則兩個都給的理由：角色有時只是一個字講錯，重骰整段反而把好的部分弄掉，還多花一次 token。

**刻意的約束**：編輯後的文字**不跑 `normalizeCharacterOutput`**——那是使用者刻意打的字，再跑一次 OpenCC 等於重現 P134 修掉的誤傷專有名詞問題（「格林格拉斯」又被改回「格林葛拉斯」）。心聲（`type === 'hv'`）不納入，它是另一種行內卡片渲染，範圍先收窄。

| 檔案 | 變更 |
|------|------|
| `services/messageEdit.js` | **新增**：純函式 `applyMessageEdit(msg, newContent)`。trim 後為空、與原內容相同、輸入型別不合皆回 `null`（呼叫端據此不寫入）；有效則回**新物件**（不就地修改原物件），且只動 `content`，`id`／`role`／`createdAt`／`charId`／`type` 一律保持原樣——這關係到 `msgId` 綁定不能斷 |
| `views/ChatRoomView.vue` | 長按選單新增「編輯內容」（條件 `role === 'assistant' && type !== 'hv' && content`）；獨立 modal（textarea，空白時儲存 disabled），文案明寫「只改這則訊息的文字，不會觸發重新生成，也不影響其他訊息」；狀態 `editContentMsg`／`editContentText` 與重傳流程的 `editingMsgRef`／`inputContent` 完全分離。既有的 `doEditAndResend`／`doRegenerate`／`isLatestUserMsg`／`isLatestAiMsg` 一行未動 |
