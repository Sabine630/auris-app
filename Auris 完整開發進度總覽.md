...
**驗證**：`keyboardViewport.test.js` 14 案例（含 safe-area 座標原點、iOS 26 resting offset、240ms 延遲更新、門檻、focus／blur／refocus、送出 interaction lock、destroy 清理）；vitest 180/180、production build 與專案設定檢查成功。390×844 本機瀏覽器實測：聊天室／貼文頁皆為單一捲動區，page／header／input bar 全為 `transform:none`，上下連續貼齊。WebKit 鍵盤合成仍以 iPhone PWA 的 dev 部署為正式放行門檻，測試全綠不可取代實機。
