...
### `services/importValidation.js`（P127）
三條 JSON 匯入路徑的單一安全規則來源；View 只負責選檔與顯示錯誤，格式、資源與 v1 schema 判定不得各自實作。

| 驗證層 | 規則 |
|--------|------|
| 讀檔前上限 | 完整備份 64 MB、角色備份 24 MB、聊天備份 32 MB；超限時不得呼叫 `file.text()` |
| 圖片 | 僅 JPEG／PNG／WebP `;base64,` data URL；宣告 MIME 必須符合 magic bytes；單張解碼後 ≤ 5 MB，另有各匯入類型圖片總量上限 |
| JSON 資源 | 限制各 store 筆數、總 records、單欄文字 200,000 字、文字總量、陣列長度、物件欄位數與巢狀深度 |
| v1 schema | keyPath 必須是非空且不重複的字串；驗證各 store 的核心字串欄位、訊息 role、`createdAt` 與群組成員型別 |

`importAllData()` 必須先讓整份備份通過本服務，再進入 P113 既有的單一 multi-store transaction。外部圖片 URL 仍由 `db.js` 的 `stripUnsafeImage()` 移除；單角色與聊天匯入只新增前置驗證，本批未改為原子 transaction。
...
- **`chatEngine.js` 新增 `getKeyDatesCtx()`**：角色生日、使用者生日、相識紀念日、在一起紀念日四個欄位，原本**只有在當天**才會經 `getPersonalDateCtx()` 注入祝賀提示；其餘 364 天模型完全看不到，被問到就自己編，編出來的答案又寫進對話歷史，反而變成它「記得」的事實（實機：使用者設定 9/3，角色堅稱 9/8，貼紀念日卡片也糾正不回來）。新函式把四個日期當**常駐既定事實**注入角色卡模板，放在 `rel_pos` 之後、`【對方資訊】` 之前。
  - 放**穩定段**（`systemStable`）：內容只隨角色卡／我的設定變化，不隨今天日期變化，故不破壞 Anthropic 的 prompt cache 點。
  - `getPersonalDateCtx()` 的當天祝賀（🎂）**保留不動**，兩段並存、語意互補：一段是「今天是你生日」，一段是「你的生日是 9 月 3 日」。該函式改為 export 僅供測試做迴歸鎖，行為未變。
  - 格式取捨：**生日只給月日不給年份**——角色年齡以角色卡既有欄位為準，給年份會讓模型自行推算而與該欄位矛盾；相識日／在一起日給完整日期，因年份決定「在一起多久」且與年齡無關。文案含四要素：必須正確回答／不可自行編造／**不主動提起也不反覆確認**（比照 `scheduleCtx` 與天氣感的措辭，避免角色沒事就報生日）／年齡以基本資訊為準。
  - 欄位無值則該項不出現；四個皆無值回空字串，不留空標題。
- **日期欄位型別防禦（順手修掉既有當機路徑）**：`fmtMonthDay`／`fmtFullDate` 的 `.split` 與 **P135 之前就存在的** `mmdd()` 的 `.slice`，在欄位不是字串時（`{}`、數字、`Date` 物件…）會拋 `TypeError`。兩者都在 `buildAIChatSetup` 必經路徑上，一拋錯該角色的聊天整個不能用，使用者只看到「錯誤：dateStr.split is not a function」。觸發途徑真實存在——`importValidation.js`／`db.js` **未對 `birthday`／`meetDate`／`togetherDate` 做型別驗證**，匯入的角色 JSON 或損壞資料即可觸發。改為先驗 `typeof` 與 `^\d{4}-\d{2}-\d{2}` 格式再取值，非法值一律回 `null`（比單純 `String()` 硬轉更嚴謹：`String({})` 會產生 `'[object Object]'`，切片後可能滲出垃圾字串）。
  - 注意這次修的是**消費端防禦**，未動輸入驗證層；匯入／存檔階段的日期型別檢查仍是既有缺口。
- 全套 Vitest 740 通過（新增 38 項），含 6 種垃圾輸入 × 兩個函式的不拋錯與不滲垃圾驗證。
...
- 新增 `importValidation.js`，在 `file.text()` 前限制完整備份 64 MB、角色 24 MB、聊天 32 MB；三條入口共用解析與安全錯誤訊息。
- 圖片改為 JPEG／PNG／WebP allowlist，驗證 `;base64,`、解碼後大小與 magic bytes；單張上限 5 MB，另限制匯入圖片總量。外部 URL、SVG、非 base64、偽造 MIME 與損毀資料均不會寫入 IndexedDB。
- v1 備份新增 store 筆數、總 records、文字／圖片總量、巢狀結構限制；keyPath 必須為非空且不重複的字串，核心欄位、role、時間戳與群組成員需符合型別。
- `importAllData` 仍使用 P113 的單一 multi-store transaction；本批只在 transaction 前補完整驗證。單角色／聊天原子化仍屬 P3 評估項目，未混入本批。
- 新增 `importValidation.test.js` 並擴充 `db.test.js`，全套 Vitest 23 個測試檔、226 項通過；包含目前教學沙盒完整資料的匯出→匯入 round-trip。
