...
iOS PWA 鍵盤問題的可證偽實機診斷層。正常網址保持惰性；只有 query 指定時才在 `html` 掛實驗 class，`kbdiag=1` 另建立即時面板。面板顯示 App 版本、body computed position/rect/scrollTop、visual viewport、頁面 rect、controller baseline/inset、focus 與 standalone/browser 模式，並以同頁連結切換實驗組合。shell 字樣只代表 query 狀態；實機結論必須同時核對 computed body 值。
